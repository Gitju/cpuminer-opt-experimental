...
static x13evo_ctx_holder x13evo_ctx;

void init_x13evo_ctx()
{
#ifdef NO_AES_NI
     sph_groestl512_init( &x13evo_ctx.groestl );
     sph_echo512_init( &x13evo_ctx.echo );
#else
     init_echo( &x13evo_ctx.echo, 512 );
     init_groestl( &x13evo_ctx.groestl );
#endif
     init_luffa( &x13evo_ctx.luffa, 512 );
     cubehashInit( &x13evo_ctx.cube, 512, 16, 32 );
     init_sd( &x13evo_ctx.simd, 512 );
     sph_blake512_init( &x13evo_ctx.blake );
     sph_bmw512_init( &x13evo_ctx.bmw );
     sph_skein512_init( &x13evo_ctx.skein );
     sph_jh512_init( &x13evo_ctx.jh );
     sph_keccak512_init( &x13evo_ctx.keccak );
     sph_shavite512_init( &x13evo_ctx.shavite );
     sph_hamsi512_init( &x13evo_ctx.hamsi );
     sph_fugue512_init( &x13evo_ctx.fugue );
}
...
static inline void x13evo_hash( void *state, const void *input )
{
   uint32_t hash[16];
   char completeCode[64];
   char resultCode[HASH_FUNC_COUNT + 1];
   x13evo_ctx_holder ctx;
   memcpy( &ctx, &x13evo_ctx, sizeof(x13evo_ctx) );
	...
	switch (idx)
	   ...
	   case 2:
	    ...
	    case 10:
	      ...
	      break;
         case 11:
	      sph_hamsi512( &ctx.hamsi, (char*)hash, size );
	      sph_hamsi512_close( &ctx.hamsi, (char*)hash );
         break;
         case 12:
	      sph_fugue512( &ctx.fugue, (char*)hash, size );
	      sph_fugue512_close( &ctx.fugue, (char*)hash );
         break;
       }
    }
...
int scanhash_x13evo( int thr_id, struct work* work, uint32_t max_nonce,
...
bool register_x13evo_algo( algo_gate_t* gate )
{
  gate->optimizations = SSE2_OPT | AES_OPT | AVX_OPT | AVX2_OPT;
  gate->scanhash  = (void*)&scanhash_x13evo;
  gate->hash      = (void*)&x13evo_hash;
  gate->hash_alt  = (void*)&x13evo_hash;
  init_x13evo_ctx();
  return true;
};
